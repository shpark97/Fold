...
from torchvision.models import (ResNet50_Weights, Swin_T_Weights,
                                ViT_B_16_Weights, RegNet_Y_16GF_Weights,
                                Wide_ResNet50_2_Weights,
                                ResNeXt50_32X4D_Weights, DenseNet121_Weights)
...
from openood.networks import (ResNet50, Swin_T, ViT_B_16, RegNet_Y_16GF,
                              WRN50_2, ResNext50, DenseNet_121)
...
if args.tvs_pretrained:
    if args.arch == 'resnet50':
        ...
        net.load_state_dict(load_state_dict_from_url(weights.url))
        preprocessor = weights.transforms()
    elif args.arch == 'wrn50':
        net = WRN50_2()
        weights = eval(
            f'Wide_ResNet50_2_Weights.IMAGENET1K_V{args.tvs_version}')
        net.load_state_dict(load_state_dict_from_url(weights.url))
        preprocessor = weights.transforms()
    elif args.arch == 'resnext50':
        net = ResNext50()
        weights = eval(
            f'ResNeXt50_32X4D_Weights.IMAGENET1K_V{args.tvs_version}')
        net.load_state_dict(load_state_dict_from_url(weights.url))
        preprocessor = weights.transforms()
    elif args.arch == 'densenet-121':
        ...
        net = DenseNet_121()
        weights = eval(f'DenseNet121_Weights.IMAGENET1K_V{args.tvs_version}')
